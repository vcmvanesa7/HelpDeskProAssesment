...
import type { IUser } from "@/schemas/user.schema"; // opcional si lo tienes
...
export async function GET(req: Request) {
  try {
    ...
    const limitDate = new Date(Date.now() - 15 * 60 * 1000); // hace 5 minutos
    ...
    for (const tk of tickets) {
      ...
      const hasAgentReply =
        assignedUser &&
        tk.messages?.some((msg) => msg.sender?.toString() === tk.assignedTo?.toString());
