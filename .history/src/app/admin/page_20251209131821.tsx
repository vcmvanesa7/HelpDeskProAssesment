...
export default function AdminDashboard() {
  ...
  // FETCH DATA usando el servicio
  useEffect(() => {
    async function load() {
      try {
        const { tickets, agents, users } = await fetchAdminDashboardData();
  ...
  const ticketsByAgent = useMemo(() => {
    ...
    agents.forEach((agent) => {
      const count = agent.tickets.length;
      map.set(agent.agent.name, count);
    });

    return Array.from(map.entries()).map(([agentName, count]) => ({
      agentName,
      count,
    }));
  }, [agents]);
  ...
  const KPI = [
    ...
    { label: "Agents", value: agents.length, link: "/admin/agents" }, // Agregar el link para Agentes
    { label: "Users", value: users.length, link: "/admin/users" }, // Agregar el link para Usuarios
  ];
        ...
        {KPI.map((card) => (
          <Grid key={card.label} item xs={12} sm={6} md={2.4}>
            ...
            >
              ...
              {card.link && (
                <Link href={card.link} style={{ display: "block", marginTop: "10px", color: "#3B82F6" }}>
                  View {card.label}
                </Link>
              )}
            </Paper>
